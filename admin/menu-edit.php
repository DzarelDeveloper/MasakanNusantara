...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $pdo->prepare(
            'UPDATE menu_items SET category = ?, name = ?, description = ?, price = ?, image = ?, serve_temp = ?, spice_option = ? WHERE id = ?'
        )->execute([$category, $name, $description, $price, $image, $serveTemp, $spiceOption, $id]);
        header('Location: menu-manage.php');
        exit;
    }

    $item = array_merge($item, compact('category', 'name', 'description', 'price', 'image', 'serveTemp', 'spiceOption'));
}

$pageTitleOwner = 'Edit: ' . $item['name'];
include __DIR__ . '/includes/layout_top.php';
?>

<div class="owner-toolbar">
    <h2 style="margin:0;">Edit: <?php echo htmlspecialchars($item['name']); ?></h2>
...
<div class="owner-modal-form" style="max-width:820px;margin:0 auto;">
    <form method="post" enctype="multipart/form-data">
        ...
        <div class="owner-field-row">
            <div class="owner-field">
                ...
                <select id="menuCategory" name="category" required>
                    <option value="Makanan" <?php echo $item['category'] === 'Makanan' ? 'selected' : ''; ?>>Makanan</option>
                    <option value="Minuman" <?php echo $item['category'] === 'Minuman' ? 'selected' : ''; ?>>Minuman</option>
                    <option value="Dimsum" <?php echo $item['category'] === 'Dimsum' ? 'selected' : ''; ?>>Dimsum</option>
                </select>
            </div>
            <div class="owner-field">
            ...
            <div class="owner-field">
                ...
                <input type="number" name="price" min="1" required value="<?php echo htmlspecialchars($item['price']); ?>">
            </div>
        </div>
        <div class="owner-field-row owner-field-row--2col">
            <div class="owner-field" id="pedasOptionField">
                <label>Pilihan Tingkat Pedas</label>
                <?php $currentSpice = $item['spice_option'] ?? 'ada'; ?>
            ...
            <div class="owner-field" id="suhuPenyajianField">
                ...
                <?php $currentTemp = $item['serve_temp'] ?? 'keduanya'; ?>
        ...
        <div class="owner-field">
            ...
            <input type="text" name="image" placeholder="assets/images/photos/..." value="<?php echo htmlspecialchars($item['image']); ?>">
        </div>
        <div class="owner-field">
